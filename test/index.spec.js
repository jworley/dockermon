...
describe('dockermon client', function() {
  ...
  describe('endpoint', function() {
    it('should accept UNIX path', function() {
      ...
      assert(mockDockerode.calledWith({
  ...
  describe('notify', function() {
    var outputPath = '/tmp/test.txt',
      outputData = 'Mary had a little bear',
      exec;
    beforeEach(function() {
      var fs = {
        existsSync: sinon.stub().withArgs(outputPath).returns(true),
        readFileSync: sinon.stub().withArgs(outputPath).returns(outputData)
      };
      exec = sinon.stub().yields();
      Dockermon.__set__('fs', fs);
      Dockermon.__set__('exec', exec);
    });

    it('should execute the given command when the file changes', function() {
      var inputData = 'Mary had a little lamb',
        _writeAndNotify = Dockermon.__get__('_writeAndNotify'),
        dockermon = new Dockermon({
          output: outputPath,
          notify: 'echo "Test"'
        });

      dockermon.outputStream = sinon.stub().yields();
      _writeAndNotify.call(dockermon, inputData);
      assert.ok(exec.calledWith('echo "Test"'));
    });
    it('should not execute the command if the file remains unchanged', function() {
      ...
      dockermon.outputStream = sinon.stub().yields();
      _writeAndNotify.call(dockermon, inputData);
      assert.ok(exec.neverCalledWith('echo "Test"'));
    });
  });
