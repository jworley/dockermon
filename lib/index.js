...
var Dockermon = module.exports = function Dockermon(options) {
  ...
  this.on('notificationNeeded', _notify.bind(this));
};
...
var _loadCallback = function() {
  ...
  fs.exists(self.input, function(exists) {
    ...
    fs.readFile(self.input, 'utf-8', function(err, fileContents) {
      if (!!err) {
        console.error('Error loading input file (%s): %s', self.input, err);
        process.exit();
      } else {
        try {
          ...
          console.log('Input (%s) loaded as module', self.input);
          self.emit('callbackLoaded');
        } catch(err) {
          console.error('Could not load input (%s) as module, encoutered error: %s', self.input, err);
          var compiled = _.template(fileContents);
          self. callback = function(docker, containers, cb) {
            cb(null, compiled({ containers: containers }));
          };

          console.log('Input (%s) loaded as template', self.input);
          self.emit('callbackLoaded');
        }
      }
    });
...
var _notify = function() {
  ...
  exec(self.notify, function(err) {
    if(!!err) {
      console.error('Error running "%s": %s', self.notify, err);
      process.exit();
    }
  });
}
  ...
  self.docker.listContainers(function(err, containers) {
    ...
    async.map(filteredContainers, function(container, cb) {
      ...
      if (!_.isEmpty(results) || self.force) {
        console.log('Generating based on %d containers', results.length)
        var outputHash;
        ...
        d.on('error', function(err) {
          console.log('Error processing input: %s', err);
          process.exit();
        })
        ...
        d.run(function() {
          self.callback(self.docker, results, d.intercept(function(data) {
            ...
            self.outputStream(data, d.intercept(function() {
              if(_.isEmpty(self.notify)) { return; }
              else {
                var dataHash = crypto.createHash('md5').update(data, 'utf-8').digest('hex');
                if (outputHash !== dataHash || _.isEmpty(outputHash)) {
                  console.log('Output (%s) has changed running notify (%s)', self.output, self.notify);
                  self.emit('notificationNeeded');
                } else {
                  console.log('Output (%s) did not change, not running notify', self.output);
                }
              }
            }));
          }));
...
Dockermon.prototype.run = function() {
  this.emit('run');
  var db_processContainers = _.debounce(_processContainers.bind(this), 250, { maxWait: 1000 });
